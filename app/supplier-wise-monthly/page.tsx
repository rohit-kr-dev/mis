...
export default function SupplierWiseMonthly() {
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Supplier Wise Monthly');
    
    // Generate filename
    const filename = `supplier_wise_monthly_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const downloadPDF = () => {
    ...
    // Save PDF
    doc.save(`supplier_wise_monthly_${new Date().toISOString().split('T')[0]}.pdf`);
  };
